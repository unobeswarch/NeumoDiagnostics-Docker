...
Run
- By default the CLI will target the auth backend at `http://auth-be:8081`. To override the API base URL set the `API_GATEWAY_URL` environment variable.

Windows (cmd.exe) example:

```cmd
set API_GATEWAY_URL=http://auth-be:8081
cargo run --release
```
...
```powershell
$env:API_GATEWAY_URL = 'http://auth-be:8081'
./cargo run --release
```
