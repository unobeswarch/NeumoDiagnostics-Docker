...
pub struct ApiClient {
    ...
    client: Client,
    // Base URL for API gateway (defaults to http://auth-be:8081)
    base_url: String,
...
impl ApiClient {
    ...
    /// `API_GATEWAY_URL` or fallback to `http://api-gateway:8080`.
    pub fn from_env() -> Result<Self> {
        let base_url = std::env::var("API_GATEWAY_URL").unwrap_or_else(|_| "http://api-gateway:8080".into());
